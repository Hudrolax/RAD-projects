...
procedure ParseHistory(GetStr: string);
...
begin
  ...
  try
    ...
    try
      ...
      //GetStr := http.Get('https://btc-e.com/api/2/btc_usd/trades');
     // ParseHistory(GetStr);
    Except
      Result := False;
    end;
...
begin
  ...
  IdMessage1.Recipients.EMailAddresses := 'coursesignal@gmail.com';
...
procedure ParseHistory(GetStr: string);
var
  OElement: TJSONObject;
  Bpair: TJSONArray;
  i, j, k, zero: Integer;
  UjeEst: Boolean;
begin
  Bpair := TJSONObject.ParseJSONValue(TEncoding.ASCII.GetBytes(GetStr), 0) as TJSONArray;
  try
    if Length(GlobalbtcUSD) = 0 then
    begin
      SetLength(GlobalbtcUSD, Bpair.Size);
      k := 0;
      for i := Bpair.Size - 1 downto 0 do
      begin
        OElement := (Bpair.Get(i) as TJSONObject);
        GlobalbtcUSD[k].typeOrder := OElement.Get('trade_type').JsonValue.Value;
        GlobalbtcUSD[k].price := StrToFloat(OElement.Get('price').JsonValue.Value);
        GlobalbtcUSD[k].amount := StrToFloat(OElement.Get('amount').JsonValue.Value);
        GlobalbtcUSD[k].tid := OElement.Get('tid').JsonValue.Value;
        GlobalbtcUSD[k].timestamp := StrToInt(OElement.Get('date').JsonValue.Value)+14400; //GMT+4
        k := k + 1;
      end;
    end
    else
      ...
      begin
        OElement := (Bpair.Get(i) as TJSONObject);
        UjeEst := false;
        for j := 0 to Length(GlobalbtcUSD) - 1 do
          if GlobalbtcUSD[j].tid = OElement.Get('tid').JsonValue.Value then
            UjeEst := true;
        if not UjeEst then
        begin
          SetLength(GlobalbtcUSD, Length(GlobalbtcUSD) + 1);
          GlobalbtcUSD[Length(GlobalbtcUSD) - 1].typeOrder := OElement.Get('trade_type').JsonValue.Value;
          GlobalbtcUSD[Length(GlobalbtcUSD) - 1].price := StrToFloat(OElement.Get('price').JsonValue.Value);
          GlobalbtcUSD[Length(GlobalbtcUSD) - 1].amount := StrToFloat(OElement.Get('amount').JsonValue.Value);
          GlobalbtcUSD[Length(GlobalbtcUSD) - 1].tid := OElement.Get('tid').JsonValue.Value;
          GlobalbtcUSD[Length(GlobalbtcUSD) - 1].timestamp := StrToInt(OElement.Get('date').JsonValue.Value)+14400; //GMT+4
        end;
      end;
  finally
  end;
  ...
  begin
   ...
   try
    ...
    begin
     ...
     OrderHistory[j].timestamp := StrToInt(OOrder.Get('timestamp').JsonValue.Value)+14400; //GMT+4
    ...
    begin
     ...
     try
      ...
      begin
       ...
       ActiveOrders[i].timestamp := StrToInt(OOrder.Get('timestamp_created').JsonValue.Value)+14400; // +4 часа для GMT+4
...
begin
  ...
  // Вычилсяем уровень максимума и минимума 5m (5 минутные)
  ...
  // Вычилсяем уровень максимума и минимума 15m (15 минутные)
  for I := Length(GlobalbtcUSD)-1 downto 0 do
  ...
  // Вычилсяем уровень максимума и минимума 30m (30 минутные)
  for I := Length(GlobalbtcUSD)-1 downto 0 do
  ...
  // Вычилсяем уровень максимума и минимума 1H (1х часовые)
  for I := Length(GlobalbtcUSD)-1 downto 0 do
  ...
  // Вычилсяем уровень максимума и минимума 4H (4х часовые)
  for I := Length(GlobalbtcUSD)-1 downto 0 do
  ...
  // Вычилсяем уровень максимума и минимума 6H (6х часовые)
  for I := Length(GlobalbtcUSD)-1 downto 0 do
  ...
  // Вычилсяем уровень максимума и минимума 12H (12х часовые)
  for I := Length(GlobalbtcUSD)-1 downto 0 do
   ...
   // Вычилсяем уровень максимума и минимума 1D (Дневные)
  for I := Length(GlobalbtcUSD)-1 downto 0 do
