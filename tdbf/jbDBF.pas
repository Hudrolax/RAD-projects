...
type
  ...
  TDBFHeader = record        { legends are from manual, changed for me specific}
    ...
    labeled2: Word                { unused, binary zero filled                }
    )
  end;
      ...
      {$ENDIF}
        ...
        try
          ...
          begin
            ...
            InternalLoad(Key, IdxBuff); {load the key}
            {vlozis ho i s pozici do seznamu}
            {with position}
            try
              tmpStrLst.Add(' ' + IdxBuff + #1 + IntToStr(I));
            except
              on EListError do
          ...
          case FDBFFields[FLD].IndexSort of
            dbfDescending: {sestupne}
              ...
              begin
                IdxBuff := tmpStrLst.Strings[I];
                L := StrToInt(Copy(IdxBuff, Pos(#1, IdxBuff) + 1, 255));
                IdxBuff := '   ' + Copy(IdxBuff, 1, Pos(#1, IdxBuff) - 1);
                Move(L, IdxBuff[1], SizeOf(L));
...
begin
  ...
  if Length(S) < Siz then
    case FDBFFields[I].FieldType of
      dbtString, dbtLogical: S := RightPad(S, Siz);
      dbtFloat, dbtNumber, dbtMemo: S := LeftPad(S, Siz);
      dbtDate: ; {date is 8 chars only ddmmyyy or mmddyyyy}
      dbtTime: ; {time is 6 chars only hhmmss}
    end;
  Move(S[1], FDBFBuff[Posic], Siz);
end;
...
begin
  ...
  {$IFNDEF VER80}
  SetLength(Rec, FDBFFields[I].FieldLength);
  {$ELSE}
  Rec[0] := Chr(FDBFFields[I].FieldLength);
  {$ENDIF}
  Move(FDBFBuff[Posic], Rec[1], FDBFFields[I].FieldLength);
end;
